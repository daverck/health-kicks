...
from app.models.cloud_models import (
    DeviceResponse,
    FallEventPage,
    FallEventResponse,
    HealthResponse,
...
from app.models.database import Device, FallEvent, HapticLog
...
def create_cloud_router(publisher: AWSIoTPublishService) -> APIRouter:
    ...
    def list_falls(
        ...
        events = (
            query.order_by(FallEvent.timestamp_utc.desc())
            .offset((page - 1) * page_size)
            .limit(page_size)
            .all()
        )
        return FallEventPage(
            items=[FallEventResponse.model_validate(event, from_attributes=True) for event in events],
            page=page,
            page_size=page_size,
            total=total,
        )
